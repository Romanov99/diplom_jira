...
#
#******************************* Поиск ключу ********************************
//include_once      'Months_Days_NDays.php';// Здесь  получаем по POST текущий номер месяца
//$Month           = $Months[$month-1];        
$KeyName         = $headers_db[0];
print "1$Month";
exit;

$QueryResult     = $dbObj->query("SELECT * FROM $dbtable WHERE $KeyName ='$KeyValue' AND Year='2018' AND Month='$Month' ORDER BY '$KeyName' ");
...
//  $QueryResult = $dbObj->query("SELECT * FROM $dbtable ORDER BY '$KeyName' ");
   ...
   {
    $msg1        ='<br>Запись  найдена в базе данных';
    if ($print){
    print '*** Tasks_Search.php - 1 ***<br>';
    print "<br>$msg1";
    print '<pre>'; 
    print_r($QueryResult); 
    print '</pre>'; 
   ...
   If ($FormType=='a_tasks_calendar')
   { 
    $Month      = $Months[$month-1];
### желательно сделать функцию и связать с базой данных ###
    print
    "
    <form action='Tasks_Search_Frame.php?KeyValue=$Proj_Id&Proj_Nm=$Proj_Nm&FormType=a_tasks_calendar' method='post'>
