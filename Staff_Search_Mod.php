...
include_once 'Header.php'                    // общий заголовок для всех страниц проекта 
?>
<?php 
####################################################################################
# Модуль производит расширенный поиск по полям -  навыки. две формы с тремя полями #
# навыков. Поиск сотрудника с тремя навыками (два поля могут быть пустыми) и       #
# поиск с альтернативными навыками (два поля могут быть пустыми)                   #
####################################################################################
#
$Arr_w                  =  array();
$Row_w                  =  array();
...
#
if     (!($_POST["skills1"]))                  // навыки 1
       $KeyValue1    = '%';                    // может быть пустым   
else   
       {
       $KeyValue1    = trim($_POST["skills1"]); 
       $KeyValue1    = '%'.$KeyValue1.'%';
       }
#
if     (!($_POST["skills2"]))                   // навыки 2
       $KeyValue2    = '%';                     // 
else                                            // 
       {
       	$KeyValue2    = trim($_POST["skills2"]);
       	$KeyValue2    = '%'.$KeyValue2.'%'; 
       }
#
if     (!($_POST["skills3"]))  
       ...
       {
       	$KeyValue3    = trim($_POST["skills3"]);// навыки 3
       	$KeyValue3    = '%'.$KeyValue3.'%';     // 
       }
# 
if (($KeyValue1=="%")&&($KeyValue2=="%")&&($KeyValue3=="%"))
   {
	...
	include_once 'View_Table_Personal_Mod.php';
	exit;
   }
else
{   
        if ($_POST[SEARCH1])   
#### Начало поиска по совокупности навыков ####       
        { 
	 ...
	 $$FormType = $dbObj->query("SELECT * FROM  $dbtable 
	                                      ...
	                                      AND   $KeyName1 LIKE '$KeyValue2'
                                              AND   $KeyName1 LIKE '$KeyValue3'
	                                      ORDER BY '$KeyName0' ");	                                      

        }
#### Конец поиска по совокупности навыков ####
        if ($_POST[SEARCH2])   
#### Начало поиска альтернативных навыков ####       
        {
#        Два поля могут быть пустыми, тогда вставим любые символы, кроме %
         if ($KeyValue1=="%") $KeyValue1='$$$';
...
#
	 ...
	 $$FormType = $dbObj->query("SELECT * FROM  $dbtable 
	                                      WHERE $KeyName1 LIKE '$KeyValue1'
	                                      OR    $KeyName1 LIKE '$KeyValue2'
                                              OR    $KeyName1 LIKE '$KeyValue3'
	                                      ORDER BY '$KeyName0' ");	                                      

        }
#### Конец поиска альтернативных навыков ####
}
...
include 'Form_Buttons_Staff_Mod.php';
print '</div>';  
//include 'Form_Buttons_Staff_Mod.php';
print "<p><a href='View_Table_Personal_Mod.php?Proj_Id=$Proj_Id&Proj_Nm=$Proj_Nm'>Просмотр всей таблицы персонала</a>";
